...
const QuizIndex = () => {
  ...
  const index = params.index ? parseInt(params.index.toString()) : 0;
  ...
  useEffect(() => {
    ...
    const quiz: QuizType = JSON.parse(storedQuiz);
    setQuiz(quiz);
  ...
  const handleNext = async () => {
    ...
    if ((index + 1) !== quiz?.questions.length && selectedOption) {
      if (index === 0) {
        ...
        const resultData: ResultType = JSON.parse(localStorage.getItem("result")!)
        const result: ResultType = {
